...
public class PDFCreator {
    ...
    private static final int fontSizeTitle = 14;
    private static final int fontSizeText = 11;
    private static final int fontSizeBullet = 18;
    ...
    public static void create(Path imagePath, FormulaireConsent formulaireConsent,String copyTo) throws Exception {
        ...
        try (PDDocument document = new PDDocument();) {
            ...
            cos.beginText();
            cos.setFont(fontBold, fontSizeTitle);
            ...
            cos.beginText();
            cos.setFont(fontPlain, fontSizeText);
            cos.newLineAtOffset(border, rect.getHeight() - 26 * (++line));
            ...
            cos.newLineAtOffset(180, 710);
            cos.showText("Oui");
            cos.endText();

            cos.beginText();
            cos.setFont(fontPlain, fontSizeText);
            cos.newLineAtOffset(350, 710);
            cos.showText("Non");
            cos.endText();

            cos.beginText();
            cos.setFont(fontPlain, fontSizeText);
            cos.newLineAtOffset(border, rect.getHeight() - 27 * (++line));
            ...
            //bloc1
            float tyBloc = rect.getHeight() - 27 * (++line);
            float spaceLine = 15;

            cos.beginText();
            cos.setFont(fontBold, fontSizeBullet);
            ...
            // Patient
            // title
            final float tyTitle = rect.getHeight() - 24 * (++line);
            cos.beginText();
            ...
            cos.moveTo(border + 45, tyTitle - 2); // moves "pencil" to a position
            cos.lineTo(border - 1, tyTitle - 2);     // creates an invisible line to another position
            cos.stroke();
            //nom
            ty = rect.getHeight() - 24 * (++line);
            cos.beginText();
            cos.setFont(fontBold, fontSizeText);
            ...
            if (formulaireConsent.getRepresentant() != null) {
                ...
                cos.endText();
                cos.setLineWidth(0.5f);
                cos.moveTo(borderRepresentant + 108, tyTitle - 2); // moves "pencil" to a position
                cos.lineTo(borderRepresentant - 1, tyTitle - 2);     // creates an invisible line to another position
                cos.stroke();
                //nom
                ty = rect.getHeight() - 24 * (line -= 2);
                cos.beginText();
                cos.setFont(fontBold, fontSizeText);
                ...
                //prenom
                ty = rect.getHeight() - 23.6F * (line += 1);
                cos.beginText();
                cos.setFont(fontBold, fontSizeText);
                cos.newLineAtOffset(borderRepresentant, ty);
                cos.showText("Prénom :");
            ...
            widgetN.setRectangle(new PDRectangle(320, 710, 10, 10));
            ...
            cos.setLineWidth(0.5f);
            cos.moveTo(50, 130); // moves "pencil" to a position
            cos.lineTo(550, 130);
            cos.moveTo(50, 330); // moves "pencil" to a position
            cos.lineTo(550, 330);
            cos.moveTo(50, 330); // moves "pencil" to a position
            cos.lineTo(50, 130);
            cos.moveTo(550, 330); // moves "pencil" to a position
            cos.lineTo(550, 130);
            cos.stroke();
            ...
            try {
                ...
                final float heightMax = 200F;
                final float widthMax = 500F;
                float x_pos = 550;
                LOG.log(Level.INFO, "x_pos" + x_pos);
                float y_pos = 330;
                LOG.log(Level.INFO, "y_pos" + y_pos);
                LOG.log(Level.INFO, "upperrighty" + page1.getCropBox().getUpperRightY());
                LOG.log(Level.INFO, "lowerLeftX" + page1.getCropBox().getLowerLeftX());

                //image est bien en landscape
                if (ximage.getWidth() > ximage.getHeight()) {
                    ...
                    float x_adjusted = (x_pos + 50 - wPhoto) / 2;
                    float y_adjusted = (y_pos + 130 - hPhoto) / 2;
                    cos.drawImage(ximage, x_adjusted, y_adjusted, wPhoto, hPhoto);
                } else {
                    LOG.log(Level.INFO, "image portrait");
                    ...
                    Matrix mt = new Matrix(0f, -1f, 1f, 0f, 100, 472);
                    cos.transform(mt);
                    cos.drawImage(ximage, x_adjusted, y_adjusted, wPhoto, hPhoto);
            ...
            SMTP.sendFormConsent(path, copyTo);
        }
    }
}
